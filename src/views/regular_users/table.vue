	...
	<el-row class="rows row2">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button v-if="user_group == '管理员' || $check_action('/regular_users/table','add') || $check_action('/regular_users/view','add')" @click="$router.push('./view?')">添加</el-button>

						

						<el-button v-if="user_group == '管理员' || $check_action('/regular_users/table','del') || $check_action('/regular_users/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="user_group == '管理员' || $check_action('/regular_users/table','set') || $check_action('/regular_users/view','set') || $check_action('/regular_users/view','get') || $check_action('/${table.tableConfig.table_nav_name}/table','add') || $check_action('/${table.tableConfig.table_nav_name}/view','add')" >


				<template slot-scope="scope">
					<div class="view_a">
					<router-link class="e-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/regular_users/table','set') || $check_action('/regular_users/view','set') || $check_action('/regular_users/view','get')"
