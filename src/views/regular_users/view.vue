		...
		<el-row class="row_ce"> 
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="用户姓名" prop="user_name">
							  ...
							  v-if="user_group === '管理员' || (form_sub['regular_users_id'] && $check_field('set','user_name')) || (!form_sub['regular_users_id'] && $check_field('add','user_name'))"></el-input>
				...
				<el-form-item label="用户性别" prop="user_gender">
						...
						v-if="user_group === '管理员' || (form_sub['regular_users_id'] && $check_field('set','user_gender')) || (!form_sub['regular_users_id'] && $check_field('add','user_gender'))">
				...
				<el-form-item label="用户年龄" prop="user_age">
							  ...
							  v-if="user_group === '管理员' || (form_sub['regular_users_id'] && $check_field('set','user_age')) || (!form_sub['regular_users_id'] && $check_field('add','user_age'))"></el-input>
				...
				<el-form-item label="用户积分" prop="user_points">
								<el-input-number id="user_points" v-model.number="form_sub['user_points']"
						v-if="user_group === '管理员' || (form_sub['regular_users_id'] && $check_field('set','user_points')) || (!form_sub['regular_users_id'] && $check_field('add','user_points'))"></el-input-number>
					<div v-else-if="$check_field('get','user_points')">{{form_sub['user_points']}}</div>
							</el-form-item>
			</el-col>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/regular_users/view','set') || $check_action('/regular_users/view','add') || $check_option('/regular_users/table','examine')">
	...
	export default {
		...
		methods: {
			...
			async submit_check(param){
				...
				var form_sub = Object.assign({} ,this.form_sub);
					...
					ret = "用户姓名不能为空!";
				} else if (!ret && !form_sub["regular_users_id"]) {
					let res = await this.$get("~/api/regular_users/count?", {"user_name": form_sub["user_name"]});
					if(res.result){
						ret = "用户姓名已存在!";
					}
				}
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["regular_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?regular_users_id=" + form_sub["regular_users_id"],form_sub,(res)=>{
