  ...
  <el-main class="bg table_wrap user">
    ...
    >
      ...
      <el-table-column prop="state" label="状态" sortable min-width="120">
        <template slot-scope="scope">{{list_state.getVal("text",{"value":scope.row.state})}}</template>
      </el-table-column>

      <el-table-column prop="login_time" label="上次登录时间" sortable min-width="200">
        <template slot-scope="scope">{{ $toTime(scope.row["login_time"],"yyyy-MM-dd hh:mm:ss") }}</template>
      </el-table-column>
