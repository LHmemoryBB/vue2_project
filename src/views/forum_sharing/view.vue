    ...
    >
      <el-row class="row_ce">
        <el-col
          v-if="
            user_group === '管理员' ||
            $check_field('get', 'share_title') ||
            $check_field('add', 'share_title') ||
            $check_field('set', 'share_title')
          "
          :xs="24"
          :sm="12"
          :lg="8"
        ...
        >
          <el-form-item label="分享标题" prop="share_title">
            <el-input
              ...
              v-if="
                user_group === '管理员' ||
                (form['forum_sharing_id'] &&
                  $check_field('set', 'share_title')) ||
                (!form['forum_sharing_id'] &&
                  $check_field('add', 'share_title'))
              "
              :disabled="disabledObj['share_title_isDisabled']"
            ></el-input>
            <div v-else-if="$check_field('get', 'share_title')">
              {{ form["share_title"] }}
            </div>
          </el-form-item>
        </el-col>
        <el-col
          v-if="
            user_group === '管理员' ||
            $check_field('get', 'sharing_categories') ||
            $check_field('add', 'sharing_categories') ||
            $check_field('set', 'sharing_categories')
          "
          :xs="24"
          :sm="12"
          :lg="8"
        ...
        >
          <el-form-item label="分享类别" prop="sharing_categories">
            <el-input
              id="sharing_categories"
              v-model="form['sharing_categories']"
              placeholder="请输入分享类别"
              v-if="
                user_group === '管理员' ||
                (form['forum_sharing_id'] &&
                ...
                (!form['forum_sharing_id'] &&
                  $check_field('add', 'sharing_categories'))
              "
              :disabled="disabledObj['sharing_categories_isDisabled']"
            ></el-input>
            <div v-else-if="$check_field('get', 'sharing_categories')">
              {{ form["sharing_categories"] }}
            </div>
          </el-form-item>
        </el-col>
        <el-col
          v-if="
            user_group === '管理员' ||
            $check_field('get', 'share_time') ||
            $check_field('add', 'share_time') ||
            $check_field('set', 'share_time')
          "
          :xs="24"
          :sm="12"
          :lg="8"
        ...
        >
          <el-form-item label="分享时间" prop="share_time">
            <el-date-picker
              :disabled="disabledObj['share_time_isDisabled']"
              v-if="
                user_group === '管理员' ||
                (form['forum_sharing_id'] &&
                  $check_field('set', 'share_time')) ||
                (!form['forum_sharing_id'] && $check_field('add', 'share_time'))
              "
              :picker-options="pickerOptions"
              id="share_time"
              v-model="form['share_time']"
              type="datetime"
              placeholder="选择日期时间"
            >
            </el-date-picker>
            <div v-else-if="$check_field('get', 'share_time')">
              {{ form["share_time"] }}
            </div>
          </el-form-item>
        </el-col>
        ...
        <el-col
          v-if="
            user_group === '管理员' ||
            $check_field('get', 'share_description') ||
            $check_field('add', 'share_description') ||
            $check_field('set', 'share_description')
          "
          :xs="24"
          :sm="12"
          :lg="8"
            ...
            <el-input
              ...
              v-if="
                ...
                (!form['forum_sharing_id'] &&
                  $check_field('add', 'share_description'))
              "
              :disabled="disabledObj['share_description_isDisabled']"
            ></el-input>
            <div v-else-if="$check_field('get', 'share_description')">
        ...
        <el-col
          v-if="
            user_group === '管理员' ||
            $check_field('get', 'sharing_content') ||
            $check_field('add', 'sharing_content') ||
            $check_field('set', 'sharing_content')
          "
          :xs="24"
          :sm="24"
          :lg="24"
        ...
        >
          <el-form-item label="分享内容" prop="sharing_content">
            <quill-editor
              v-model.number="form['sharing_content']"
              v-if="
                user_group === '管理员' ||
                (form['forum_sharing_id'] &&
                  $check_field('set', 'sharing_content')) ||
            ...
            <div
              v-else-if="$check_field('get', 'sharing_content')"
              v-html="form['sharing_content']"
            ></div>
          </el-form-item>
        </el-col>
        <el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
          <el-form-item label="审核状态" prop="examine_state">
            <el-select
              ...
              v-if="
                user_group === '管理员' ||
                (form['examine_state'] && $check_examine()) ||
                (!form['examine_state'] && $check_examine())
              "
            >
      ...
      <el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
        <el-form-item
          v-if="
            $check_action('/forum_sharing/view', 'set') ||
            $check_action('/forum_sharing/view', 'add') ||
            $check_option('/forum_sharing/table', 'examine')
          "
        >
...
export default {
  ...
  methods: {
    ...
    get_obj_before(param) {
      ...
      if (
        this.form["share_time"] &&
        this.form["share_time"].indexOf("-") === -1
      ) {
        this.form["share_time"] = this.$toTime(
          parseInt(this.form["share_time"]),
          "yyyy-MM-dd hh:mm:ss"
        );
      }
      $.db.del("form");
