  ...
  <el-main class="bg table_wrap nav">
      ...
      <el-row class="rows row2">
        <el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap">
          <el-col :xs="24" :sm="12" :lg="12" class="search_btn_1 btns">
            ...
            <el-button
              v-if="
                user_group == '管理员' || $check_action('/nav/table', 'del')
              "
            ...
            <el-button
              v-if="user_group == '管理员' || $check_action('/nav/view')"
    ...
    >
      ...
      <el-table-column prop="target" label="跳转方式" sortable min-width="120">
        <template slot-scope="scope">
          {{
            getObj(list_target, scope.row.target).text ||
            (scope.row.target ? "自定义" : "本页面")
          }}
        </template>
      </el-table-column>
